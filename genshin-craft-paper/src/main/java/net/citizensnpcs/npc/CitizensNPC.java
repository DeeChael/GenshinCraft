...
public class CitizensNPC extends AbstractNPC {
    ...
    public boolean despawn(DespawnReason reason) {
        ...
        if (reason == DespawnReason.CHUNK_UNLOAD) {
            event.setCancelled(data().get(Metadata.KEEP_CHUNK_LOADED, Setting.KEEP_CHUNKS_LOADED.asBoolean()));
        }
    ...
    public boolean isUpdating(NPCUpdate update) {
        return update == NPCUpdate.PACKET
                ? updateCounter > data().get(Metadata.PACKET_UPDATE_DELAY, Setting.PACKET_UPDATE_DELAY.asTicks())
                : false;
    }
    ...
    public void save(DataKey root) {
        super.save(root);
        if (!data().get(Metadata.SHOULD_SAVE, true))
            return;
    ...
    public void scheduleUpdate(NPCUpdate update) {
        if (update == NPCUpdate.PACKET) {
            updateCounter = data().get(Metadata.PACKET_UPDATE_DELAY, Setting.PACKET_UPDATE_DELAY.asTicks()) + 1;
        }
    }
        ...
        Consumer<Runnable> postSpawn = new Consumer<Runnable>() {
            ...
            public void accept(Runnable cancel) {
                ...
                if (type.isAlive()) {
                    ...
                    } else if (data().has(Metadata.AGGRESSIVE)) {
                        NMS.setAggressive(entity, data().<Boolean> get(Metadata.AGGRESSIVE));
                    }

                    if (SUPPORT_NODAMAGE_TICKS && (Setting.DEFAULT_SPAWN_NODAMAGE_DURATION.asTicks() != 20
                            || data().has(Metadata.SPAWN_NODAMAGE_TICKS))) {
                        try {
                            entity.setNoDamageTicks(data().get(Metadata.SPAWN_NODAMAGE_TICKS,
    ...
    public void update() {
        try {
            ...
            if (data().has(Metadata.ACTIVATION_RANGE)) {
                int range = data().get(Metadata.ACTIVATION_RANGE);
            ...
            boolean shouldSwim = data().get(Metadata.SWIMMING, SwimmingExaminer.isWaterMob(getEntity()))
            ...
            if (navigator.isNavigating()) {
                if (shouldSwim) {
                    getEntity().setVelocity(getEntity().getVelocity().multiply(
                            data().get(Metadata.WATER_SPEED_MODIFIER, Setting.NPC_WATER_SPEED_MODIFIER.asFloat())));
            ...
            if (SUPPORT_GLOWING && data().has(Metadata.GLOWING)) {
                try {
                    getEntity().setGlowing(data().get(Metadata.GLOWING, false));
                } catch (NoSuchMethodError e) {
                    SUPPORT_GLOWING = false;
                }
            }

            if (SUPPORT_SILENT && data().has(Metadata.SILENT)) {
                try {
                    getEntity().setSilent(Boolean.parseBoolean(data().get(Metadata.SILENT).toString()));
                } catch (NoSuchMethodError e) {
            ...
            if (isUpdating(NPCUpdate.PACKET)) {
                if (data().get(Metadata.KEEP_CHUNK_LOADED, Setting.KEEP_CHUNKS_LOADED.asBoolean())) {
            ...
            if (isLiving) {
                ...
                if (SUPPORT_PICKUP_ITEMS) {
                    try {
                        ((LivingEntity) getEntity()).setCanPickupItems(data().get(Metadata.PICKUP_ITEMS, false));
                    } catch (Throwable t) {
                ...
                if (getEntity() instanceof Player) {
                    ...
                    if (data().has(Metadata.SNEAKING) && !hasTrait(SneakTrait.class)) {
    ...
    private void updateCustomNameVisibility() {
        String nameplateVisible = data().<Object> get(Metadata.NAMEPLATE_VISIBLE, true).toString();
    ...
    private void updateFlyableState() {
        ...
        if (!Util.isAlwaysFlyable(type))
            return;
        if (!data().has(Metadata.FLYABLE)) {
            data().setPersistent(Metadata.FLYABLE, true);
        }
    ...
    private void updateScoreboard() {
        if (data().has(Metadata.SCOREBOARD_FAKE_TEAM_NAME)) {
            getOrAddTrait(ScoreboardTrait.class).update();
        }
    }

    private void updateUsingItemState(Player player) {
        boolean useItem = data().get(Metadata.USING_HELD_ITEM, false),
                offhand = data().get(Metadata.USING_OFFHAND_ITEM, false);
        if (!SUPPORT_USE_ITEM)
            return;
        try {
            if (useItem) {
